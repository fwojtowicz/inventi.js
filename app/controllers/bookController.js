                    ...
                    .then((authorData) => {
                      ...
                      Book.findOrCreate({
                        ...
                        .then((bookData) => {
                          ...
                          if (authorData[0]) {
                            console.log('AUTHORDATA', authorData.length.length)
    ...
    .then((ownedBookData) => {
      ...
      Book.findByPk(ownedBookData.dataValues.book_id).then((bookData) => {
        ...
        for (var j = 0; j < authors.length; j++) {
          console.log('AUTHOROBJECT', authors[j])
          var newAuthorsPromise = Author.update(
            {
              author_name: authors[j].author_name,
              author_surname: authors[j].author_surname
            },
            {
              where: {
                author_id: authors[j].BookAuthors.author_id
              }
            }
          )
          authorData.push(newAuthorsPromise)
        }
        return Promise.all(authorData).then((authorData) => {
          ...
          genreData = []
          for (var g = 0; g < genres.length; g++) {
            var newGenresPromise = Genre.update(
              {
                genre_name: genres[g].genre_name
              },
              {
                where: {
                  genre_id: genres[g].BookGenres.genre_id
                }
              }
            )
            genreData.push(newGenresPromise)
          }
          return Promise.all(genreData).then(
            (genreData) => {
              if (!genreData) {
                console.log('genreData details error')
              }
              // bookData[0].setGenres(genreData)
